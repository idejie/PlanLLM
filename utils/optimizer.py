...
def add_different_lr(named_param_tuples_or_model, diff_lr_module_names_lrs, default_lr):
...
def create_optimizer(args, model, filter_bias_and_bn=True, return_group=False):
    ...
    if hasattr(args, "different_lr") and args.different_lr.enable:
        diff_lr_module_names_lrs = args.different_lr.modules_lrs
    else:
        diff_lr_module_names_lrs = dict()
    ...
    named_param_tuples = add_different_lr(
        named_param_tuples, diff_lr_module_names_lrs, args.lr)
