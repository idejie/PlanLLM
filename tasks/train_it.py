...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'VideoChat2_vicuna_it'))
    (
        model,
        model_without_ddp,
        optimizer,
            ...
            # save the last model to logger path
                torch.save(
                    ...
                    os.path.join(config.saved_path,
                        f"{config.dataset}/T{config.max_traj_len}_model_it_last.pth"  
                    )
                )
                ...
                if SR > max_SR:
                    ...
                    log_save_path = f"{config.dataset}_T{config.max_traj_len}_model_it_best.pth"  
                    checkpoint_save_path = os.path.join(
                            config.saved_path, 
                            config.dataset,
                            f"T{config.max_traj_len}_model_it_best.pth"
                        )
