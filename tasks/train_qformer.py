...
from models.planllm_qformer import PlanLLM_qformer
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            text_input = tokenizer(
                ...
                padding="max_length",
                truncation=True,
                max_length=args.inputs.max_txt_l,
                return_tensors="pt",
            ).to(
                device
...
def train(
    ...
    loss_names =['action','state_encode','task','state_decode','loss_vtc','loss_vtm','loss_cap']
    ...
    for i, (media_type, (data)) in enumerate(iterator):
        ...
        text_input = tokenizer(
            ...
            padding="max_length",
            truncation=True,
            max_length=config.inputs.max_txt_l,
            return_tensors="pt",
        ).to(
            device
        )  # change from "longest" to "max_length"
        ...
        with torch.cuda.amp.autocast(enabled=config.fp16):
            ...
            loss+=losses['loss_vtc']+losses['loss_vtm']+losses['loss_cap']
        ...
        if is_main_process() and config.wandb.enable and global_step % log_freq == 0:
            # logger.info("Epoch: {} State Loss: {:.2f} Top1 Acc: {:.2f}%"\
            #         .format(epoch+1, losses_state.avg, state_acc.avg))
            # logger.info("\tAction Loss: {:.2f}, SR: {:.2f}% Acc1: {:.2f}% Acc5: {:.2f}% MIoU: {:.2f}"\
            #             .format(losses_action.avg,
            #                     action_sr.avg,
            #                     action_acc1.avg,
...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'PlanLLM_qformer'))
    ...
    for epoch in range(start_epoch, config.scheduler.epochs):
        if not config.evaluate or not config.eval:
            global_step = train(
                model,
                train_loaders,
                optimizer,
            ...
            if is_main_process():
                ...
                torch.save(
                    ...
                    os.path.join(config.saved_path,
                        f"{config.dataset}/T{config.max_traj_len}_model_qformer_last.pth"  
                    )
                )
                ...
                if SR > max_SR:
                    ...
                    log_save_path = f"{config.dataset}_T{config.max_traj_len}_model_qformer_best.pth"  
                    checkpoint_save_path = os.path.join(
                            config.saved_path, 
                            config.dataset,
                            f"T{config.max_traj_len}_model_qformer_best.pth"
                        )
