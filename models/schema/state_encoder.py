...
class StateEncoder(nn.Module):
    def __init__(self, vis_input_dim, lang_input_dim, embed_dim, dropout=0.1):
        ...
        state_interact_layer = nn.TransformerEncoderLayer(d_model=vis_input_dim,dim_feedforward=vis_input_dim, nhead=8,batch_first=True,dropout=dropout)
    ...
    def forward(self, state_feat, all_state_desc_feat):
        ...
        # n_action,1(start),n_desc,d
        start_desc_feat = prompt_features[:, :num_desc // 2, :]
        # n_action,1(end),n_desc,d
        ...
        # Improvement1: add state interaction
        state_feat = self.state_interaction(state_feat)
